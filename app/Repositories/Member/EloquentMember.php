...
class EloquentMember implements MemberRepository
{
    ...
    public function __construct(MemberAvatarManager $avatarManager, RoleRepository $roles)
    {
    ...
    {
        return Member::leftJoin('social_logins', 'users.id', '=', 'social_logins.user_id')
            ->select('users.*')
    ...
    {
        return Member::leftJoin('sessions', 'users.id', '=', 'sessions.user_id')
            ->select('users.*')
    ...
    public function delete($id)
    {
        ...
        $this->avatarManager->deleteAvatarIfUploaded($member);
    ...
    {
        return Member::whereBetween('created_at', [Carbon::now()->firstOfMonth(), Carbon::now()])
            ->count();
    }
    ...
    {
        return Member::orderBy('created_at', 'DESC')
            ->limit($count)
            ->get();
    }
    ...
    {
        $result = Member::whereBetween('created_at', [$from, $to])
            ->orderBy('created_at')
            ->get(['created_at'])
    ...
    {
        return Member::where('confirmation_token', $token)->first();
    }
