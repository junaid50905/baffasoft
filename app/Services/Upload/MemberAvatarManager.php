...
class MemberAvatarManager
{
     ...
     * @return bool
     */
    private function userHasUploadedAvatar(Member $member)
    {
        return $member->avatar && ! Str::contains($member->avatar, ['http', 'gravatar']);
    }
     ...
     * @param Member $member
     */
    public function deleteAvatarIfUploaded(Member $member)
    {
        if (! $this->memberHasUploadedAvatar($member)) {
            return;
        }
