...
<!--                                    <option value="Active">Active</option>-->
                ...
                <div class="table-responsive" id="users-table-wrapper">
                    ...
                    >
                        ...
                        <template #cell(any_change)="row">
                            {{ row.value == '1' ? 'Yes - ' + row.item.structure_change_charge + 'TK' : 'No' }}
                        </template>
                        ...
                        <template #cell(member)="row">
                            ...
                            <span v-if="checkAccess('edit_renew_member')">
                                ...
                                <b-button v-if="row.item.status == 'Pending'"
                                    v-b-modal.modal-member-approval
                                    @click="sendActiveMemberInfo(row.item.member_id)"
                                    variant="icon"
...
<!--                                <span v-if="checkAccess('approve_renew_member')">
                                    <button v-if="row.item.status == 'Wait For Approval'"
                                            class="btn btn-success btn-sm"
                                            @click="approveMember(row.item.id)"> Approved </button>
                                </span>-->
                                <span v-if="checkAccess('payment_renew_member')">
                                    <span v-if="row.item.status == 'Accepted'">
                                        <router-link v-if="!row.item.is_payment"
                                             ...
                                             props_receipt_type: 'membership_annual_subscription',
                                             props_receipt_year: row.item.submission_year, }, }"
                                                     class="btn btn-success btn-sm">Payment</router-link>
                                        <b-button v-if="row.item.any_change"
                                            v-b-modal.modal-member-approval1
                                            @click="sendInfo(row.item.id)"
                                            variant="icon"
                                            title="Company Structural change Amount"
                                            data-toggle="tooltip"
                                            data-placement="top"
                                        ><i class="fas fa-cogs text-info"></i></b-button>
                                    </span>
                                </span>
<!--                                <span v-if="checkAccess('active_renew_member')">
                                    <button v-if="row.item.status == 'Paid'" class="btn btn-success btn-sm" @click="activeMember(row.item.id)"> Active </button>
                                </span>-->
                        </template>
                    </b-table>
                </div>
