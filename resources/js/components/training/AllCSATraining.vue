<template>
    <div class="container"><div class="row"><div class="col-sm-12"><div class="card">
        <div class="card-header text-light bg-secondary bg-gradient"><h4 class="text-center">Cargo Security Awarness Training</h4></div>
        <div class="card-body">
        <validation-errors
        ...
        >
          ...
          <router-link
              title="Training"
              :to="{ name: 'training_csa', params: { id: row.item.id } }"
              class="btn btn-secondary btn-sm"
          >
            View
          </router-link>
                ...
                <span v-if="checkAccess('approve-training')">
                    <button v-if="row.item.status === 'Pending' ? true : false"
                            @click="verifyMember(row.item.id)" class="btn btn-warning btn-sm">Verify
                                    </button>
                    <!--                    <b-button v-if="row.item.status === 'Pending' ? true : false" v-b-modal.modal-approval
                                                  @click="sendInfo(row.item.id,row.item.previous_caab_id_no)" class="btn btn-warning btn-sm">
                                            Approval
                                        </b-button>-->
                </span>
          ...
          <router-link
              ...
              query: {
                  ...
                  props_receipt_type: 'csa_training',
          ...
          >
              ...
              <b-button
                  ...
                  @click="sendInfo(row.item.id, row.item.previous_caab_id_no)"
        ...
        >
            <form ref="form" @submit.stop.prevent="handleUpdate">
                <b-form-group
                    ...
                    invalid-feedback="Certificate Number is required"
                    :state="nameState"
                >
                    <b-form-input
                ...
                <b-form-group label="Previous CAAB ID Number" label-for="previous_caab_id_no">
                    <b-form-input
                        id="previous_caab_id_no"
                        v-model="previous_caab_id_no"
                        disabled
                    ></b-form-input>
                </b-form-group>
                <b-form-group
                    label="New CAAB ID Number"
                    label-for="caab_id_no"
                    invalid-feedback="Comment is required"
                >
                    <b-form-input id="caab_id_no" v-model="caab_id_no"></b-form-input>
                </b-form-group>
                <b-form-group label="Certificate Validity" label-for="certificate_validity">
                    <b-form-datepicker
                        id="certificate_validity"
                        v-model="certificate_validity"
                        class="mb-2"
                    ></b-form-datepicker>
                    <!--                    <b-form-input id="certificate_validity" v-model="certificate_validity"></b-form-input>-->
                </b-form-group>
                <b-form-group label="Delivery Date" label-for="delivery_date">
                    <b-form-datepicker
                        id="delivery_date"
                ...
                <b-form-group
                    ...
                    invalid-feedback="File number is required"
                >
                    <b-form-input id="file_number" v-model="file_number"></b-form-input>
                </b-form-group>
            </form>
        </b-modal>
...
export default {
    ...
    data: () => ({
        ...
        caab_id_no: null,
        certificate_validity: new Date().toISOString().slice(0, 10),
        delivery_date: new Date().toISOString().slice(0, 10),
        file_number: null,
        header: ["Cargo Security Awareness Training"],
        ...
        json_fields: {
            ...
            "Card Holder's Name": "participant_name",
            Designation: "participant_designation",
            "Name of the Organization": "member",
            "CAAB ID": "previous_caab_id_no",
            "Previous File No.": "previous_caab_id_no",
            "Police Verification Issue Date": "applicant_police_verification_date",
            "Passport/NID No./ Birth Certificate No.": "applicant_national_id_card_number",
            "Mobile No.": "participant_mobile",
            "Company email": "participant_email",
    ...
    methods: {
        ...
        resetModal() {
            ...
            this.previous_caab_id_no = null;
            this.certificate_number = null;
            this.caab_id_no = null;
            this.certificate_validity = null;
        ...
        handleUpdate() {
            ...
            axios
                .put("/api/training/" + this.training_id, {
        ...
        getCompanyOwners() {
            let url = '/api/training?training_name=csa';
            if (this.is_member && this.user)
                url = '/api/training?training_name=csa&member_id=' + this.user.id
            axios
                .get(url)
                .then((res) => {
                    this.company_owners = res.data.data;
