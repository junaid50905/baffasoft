<template>
    <div class="container"><div class="row"><div class="col-sm-12"><div class="card">
        <div class="card-header text-light bg-secondary bg-gradient"><h4 class="text-center">Dangerous Goods Regulations Training</h4></div>
        <div class="card-body">
        ...
        <b-row v-if="!is_member">
            ...
            <b-col sm="3" md="3" class="my-1">
                <b-input-group>
                    <b-form-input v-model="select_participant" name="select_participant" type="search"
                                  placeholder="Participant Name"></b-form-input>
                </b-input-group>
            </b-col>
...
<!--                <span v-if="checkAccess('edit-training')">
                    <router-link
                        title="Training"
                        :to="{name: 'edit_training_csa',params:{'id':row.item.id}}" class="btn btn-secondary btn-sm">
                    Edit
                </router-link>
                </span>-->
                <span v-if="checkAccess('view-training')">
                    <router-link
                        title="Training"
                        :to="{name: 'training_dg',params:{'id':row.item.id}}" class="btn btn-secondary btn-sm">
                    View
                </router-link>
                </span>
...
<!--                    <b-button v-if="row.item.status === 'Pending' ? true : false" v-b-modal.modal-approval
                ...
                <span v-if="checkAccess('payment-training')">
                    <router-link v-if="row.item.status === 'Verified' ? true : false"
                                 :to="{
                        name: 'new-collection',
                                 params: {member_id: row.item.member_id },
                                 query: {
                            props_id: row.item.id,
                            props_receipt_type: 'dgr_training',
                            props_receipt_year: row.item.submission_year
                        }}"
                                 class="btn btn-success btn-sm">Payment</router-link>
                </span>
                <a v-if="row.item.is_payment" class="btn btn-primary btn-sm" target="_blank" :href="addProjectPath('/print_money_receipt/'+ row.item.money_collection_id)">Voucher</a>
            </span></template>
...
export default {
    ...
    data: () => ({
        ...
        header: ['Dangerous Goods Regulations'],
        ...
        fields: [
            {
                ...
                label: 'Organization',
                sortable: true,
            }, {key: 'category_name', label: 'Category'}, 'training_name', {key: "submission_year", label: 'Year'},{key:'participant_name',label:'Name'},{key:'participant_mobile',label:'Phone'},'status','created_at','actions'],
        json_fields: {
            'YEAR': {
            ...
            'Academic Qualification': "participant_qualification",
    ...
    methods: {
            ...
            if (end_date >= start_date) {
                ...
                axios
                    .get('/api/training?' +
                        ...
                        '&end_date=' + end_date +
                        '&member_id=' + this.selected_bmn_no +
                        "&submission_year=" + this.submission_year +
                        '&participant_name=' + this.select_participant +
        ...
        getCompanyOwners() {
            let url = '/api/training?training_name=dg';
            if (this.is_member && this.user)
                url = '/api/training?training_name=dg&member_id=' + this.user.id
            axios
                .get(url)
                .then(res => {
                    this.company_owners = res.data.data
